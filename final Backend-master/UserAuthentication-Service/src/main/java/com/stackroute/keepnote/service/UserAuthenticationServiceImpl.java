...
public class UserAuthenticationServiceImpl implements UserAuthenticationService {
	...
	public boolean saveUser(User user) throws UserAlreadyExistsException {
		/*
		 * if(userAuthenticationRepository.existsById(user.getUserId())) throw new
		 * UserAlreadyExistsException("Cannot Register User"); else {
		 */

		try {
			Optional<User> usr = userAuthenticationRepository.findById(user.getUserId());
			if (!usr.isPresent()) {
